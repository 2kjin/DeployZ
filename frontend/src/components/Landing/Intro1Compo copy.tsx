...
import whiteCar from "@/assets/3dmodel/factory.glb"
import grayCar from "@/assets/3dmodel/factory2.glb"

const ZoomInOut = ({size} : {size: React.MutableRefObject<HTMLDivElement>}) => {
  ...
  /** 카메라 커스텀 함수 */
  const SetupCamera = () => {
    const width = size.current.clientWidth;
    const height = size.current.clientWidth;
  ...
  useEffect(() => {
    if (divContainer.current) {
      ...
      scene.current = scn;
      scn.background = new THREE.Color(`${theme.colors.container}`);
