...
public class CustomOAuth2UserService implements OAuth2UserService<OAuth2UserRequest, OAuth2User> {
	...
	public SocialType getSocialType(final String registrationId) {
		if (SocialType.GITLAB.getSocialType().equals(registrationId)) {
	...
	public Member getUser(final OAuthAttributes extractAttributes, final SocialType socialType) {
		final Optional<Member> memberByRegistrationIdAndEmail = memberRepository.findMemberByRegistrationIdAndEmail(
			socialType.getSocialType(), extractAttributes.getOAuth2MemberInfo().getEmail());
	...
	private Member saveUser(final OAuthAttributes extractAttributes, final SocialType socialType) {
		final Member member = extractAttributes.toEntity(socialType, extractAttributes.getOAuth2MemberInfo());
